(function() {
    ...
    return {
        get: function(req, s, headers) {
            ...
            var r = Data.fetch('select nick_name,private_key from user where id=' + owner);
            if (r.hasOwnProperty('error') || r.data.length == 0) {
                Log.error('internal error, unknown owner = ' + owner);
                return {error: 454, detail: 'not login'};
            }

            return {error: 0, detail: r.data[0][0], extra: r.data[0][1]};
        },
        post: function(req, s, headers) {
            ...
            Data.useDataSource('mysql_any_badge');
            var r = Data.fetch('select id,private_key from user where name="' + req.name + '" and password="' + req.password + '"');
            if (r.data.length == 0) {
                return {error:453, detail: 'user not exist or bad password'};
            }

            var key = 'any-badge-session-' + randomString(16) + '-' + (+new Date);
            Cache.aging.put(key, r.data[0][0], req.remember ? 99999999999999 : 30000);

            return {error: 0, detail: key, extra: r.data[0][1]};
        }
    }
})();